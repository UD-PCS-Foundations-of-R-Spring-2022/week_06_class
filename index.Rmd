...
How many NAs are in each field?
...
How many unique values are there by field?
...
```{r}
this_is_a_string <- "'Hello! String!' said the adjuct professor"
```

```{r}
single_in_single <- 'I\'m using single quotes'
```
...
foia_log_OMB <- 
  ...
  mutate(internally_reviewed_by = str_remove_all(string = internally_reviewed_by, 
                                      pattern = "[^A-Za-z]"),
         internally_reviewed_by = if_else(is.na(internally_reviewed_by), "None", internally_reviewed_by))
```
...
foia_log_OMB <-
  ...
  mutate(division = case_when(
    ...
    str_detect(string = division, pattern = "Benefits") ~ "Benefits Office",
...
Create `all_requested_records_sent_flag`
```{r}
foia_log_OMB <-
  foia_log_OMB %>%
